...
#  ORGANIZATION: 
#       CREATED: 2013/8/2 14:42:35 中国标准时间
#      REVISION:  ---
#===============================================================================
...
#---  FUNCTION  ----------------------------------------------------------------
#          NAME:  Out_msg_head
#   DESCRIPTION:  输出安全加固项及其描述
#    PARAMETERS:  $1:第几项；$2:描述；
#       RETURNS:  无
#-------------------------------------------------------------------------------
...
#   DESCRIPTION:  检查今天是否已经备份过配置文件，如果没有则备份
#    PARAMETERS:  $1:配置文件 
#       RETURNS:  
#-------------------------------------------------------------------------------
...
function Check_Linux_2(){
    ...
    Out_msg_head 2 "删除或锁定与设备运行、维护等工作无关的账号。"
...
function Check_Linux_3(){
    ...
    Out_msg_head 3 "限制具备超级管理员权限的用户远程登录";
...
function Check_Linux_4(){
    ...
    Out_msg_head 4 "根据系统要求及用户的业务需求，建立多帐户组，将用户账号分配到相应的帐户组。"
	#do not something;
    echo "请手动检查"
    Out_msg_end;
	return 0;
...
function Check_Linux_5(){
    Out_msg_head 5 "对系统账号进行登录限制，确保系统账号仅被守护进程和服务使用，不应直接由该账号登录系统。如果系统没有应用这些守护进程或服务，应删除这些账号。"
	#do not something;
    echo "请手动检查";
    Out_msg_end;
	return 0;
...
function Check_Linux_6(){
    ...
    Out_msg_head 6 "对于采用静态口令认证技术的设备，口令长度至少8位，并包括数字、小写字母、大写字母和特殊符号4类中至少3类。"
...
function Check_Linux_7(){
	    ...
	    Out_msg_head 7 "对于采用静态口令认证技术的设备，帐户口令的生存期不长于90天。"
...
function Check_Linux_8(){
		Out_msg_head 8 "对于采用静态口令认证技术的设备，应配置当用户连续认证失败次数超过6次（不含6次），锁定该用户使用的账号。"
...
function Check_Linux_9(){
	Out_msg_head 9 "在设备权限配置能力内，根据用户的业务需要，配置其所需的最小权限。"
...
function Check_Linux_10(){
	Out_msg_head 10 "控制用户缺省访问权限，当在创建新文件或目录时 应屏蔽掉新文件或目录不应有的访问允许权限。\n防止同属于该组的其它用户及别的组的用户修改该用户的文件或更高限制";
...
function Check_Linux_14(){
	    Out_msg_head 14 "[可选]设备配置远程日志功能，将需要重点关注的日志内容传输到日志服务器。";
...
function Check_Linux_15(){
	    Out_msg_head 15 "对于使用IP协议进行远程维护的设备，设备应配置使用SSH等加密协议，并安全配置SSHD的设置。";
...
function Check_Linux_18(){
        Out_msg_head 18 "主机系统应该禁止ICMP重定向，采用静态路由"
...
function Check_Linux_19(){
    Out_msg_head 19 "对于不做路由功能的系统，应该关闭数据包转发功能。"
...
function Check_Linux_20(){
    Out_msg_head 20 "对于具备字符交互界面的设备，应配置定时帐户自动登出";
...
function Check_Linux_21(){
    Out_msg_head 21 "对于具备图形界面（含WEB界面）的设备，应配置定时自动屏幕锁定。"
...
function Check_Linux_22(){
    Out_msg_head 22 "涉及账号、账号组、口令、服务等的重要文件和目录的权限设置不能被任意人员删除，修改。"
...
function Check_Linux_23(){
    Out_msg_head 23 "应该从应用层面进行必要的安全访问控制，比如FTP服务器应该限制ftp可以使用的目录范围。"
...
function Check_Linux_24(){
    Out_msg_head 24 "在系统安装时建议只安装基本的OS部份，其余的软件包则以必要为原则，非必需的包就不装。"
...
function Check_Linux_25(){
    Out_msg_head 25 "应根据需要及时进行补丁装载。对服务器系统应先进行兼容性测试。"
...
function Check_Linux_26(){
    Out_msg_head 26 "如果网络中存在信任的NTP服务器，应该配置系统使用NTP服务保持时间同步。"
...
function Check_Linux_27(){
    Out_msg_head 27 "NFS服务：如果没有必要，需要停止NFS服务；如果需要NFS服务，需要限制能够访问NFS服务的IP范围。"
    echo -e "
停止NFS服务
Service nfs stop
...
function Check_Linux_29(){
    Out_msg_head 29 "列出系统启动时自动加载的进程和服务列表，不在此列表的需关闭。";
...
function Check_Linux_30(){
    Out_msg_head 30 "监视服务器的CPU、硬盘、内存、网络等资源。";
...
function Check_Linux_31(){
    Out_msg_head 31 "列出所需要服务的列表(包括所需的系统服务)，不在此列表的服务需关闭。"
